...
uses
  ...
  Vcl.Buttons, unPesquisaPessoas, unDataModule, Data.DB, Data.Win.ADODB, System.StrUtils,Math,
  unFuncoes;
...
begin
  ...
  if Trim(edtRazaoSocial.Text) = '' then begin
    alerta('Informe a razão social da pessoa.');
  ...
  if (Not ckCliente.Checked) and (not ckFornecedor.Checked) then begin
    alerta('Selecione ao menos 1 (uma( das opções: Cliente / Fornecedor.');
    ckCliente.SetFocus;
    Abort;
  end;
  ...
  if not(emailValido(edtEmail.Text)) then begin
    alerta('Email informado inválido');
    edtEmail.SetFocus;
    Abort;
  end;
  ...
  qryPessoa.Parameters.ParamByName('pesTipo').Value := IfThen(rdgTipoPessoa.ItemIndex = 0, 'F', 'J');
...
begin
  ...
  begin
    ...
    //IfThen com números inteiros precisa declarar StrUtils e Math
